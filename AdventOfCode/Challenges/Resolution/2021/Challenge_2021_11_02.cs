...
{
    public class Challenge_2021_11_02 : IChallengeResolution
    {
        ...
        public string ResolveChallenge(List<string> data)
        {
            ...
            var dataCount = intData.SelectMany(c => c).Count();

            var stepCount = 0L;

            while (true) // Never give up, never surrender!
            {            // Keep going until we find an instance where all elements have synchronized flashing
                ...
                var flashCount = flashed.Sum(c => c.Value.Count);

                if (flashCount == dataCount)
