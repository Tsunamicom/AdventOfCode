...
{
    public class Challenge_2021_04_02 : IChallengeResolution
    {
        ...
        public string ResolveChallenge(List<string> data)
        {
            ...
            var winnerBoardIds = new HashSet<int>();

            for (int i = 0; i < bingoCallsAll.Count; i++)
            {
                if (winnerBoardIds.Count == bingoCards.Count) break;
                ...
                {
                    if (!winnerBoardIds.Contains(j))
                    {
                        if (IsHorizontalMatch(bingoCards[j], allCalled) || IsVerticalMatch(bingoCards[j], allCalled))
                        {
                            winnerBoardIds.Add(j);
                        }
                    }
                }
            };

            var lastWinnerId = winnerBoardIds.LastOrDefault(-1);
            if (lastWinnerId != -1)
            {
                var unmarkedValue = CalculateUnmarkedValueSum(bingoCards[lastWinnerId], allCalled);

                return (unmarkedValue * allCalled.Last()).ToString();
            }
